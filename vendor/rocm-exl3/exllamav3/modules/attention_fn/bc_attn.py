...
def _compile_kernel(device: torch.device, fn, signature: dict, constexprs: dict,
    ...
    if k is None:
        ...
        with torch.cuda.device(device):
            ...
            # The existing HIP driver shim consumes an AMD code object, not
            # a CUDA cubin. Its launch bridge currently assumes 32-lane waves.
            if torch.version.hip and getattr(ck.metadata, "warp_size", 32) != 32:
                raise ValueError("Native attention bridge requires a 32-lane AMD target")
            binary = ck.asm["hsaco" if torch.version.hip else "cubin"]
