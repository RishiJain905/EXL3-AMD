...
def _long_qc_decode_options(args, k_bits, v_bits):
    """Opt-in measured geometry; all other attention keeps inherited defaults.

    Use metadata only; never synchronize a GPU cache length for dispatch.
    The generator pads target tables to 16-page multiples. For this single
    batch, physical cache pages cap the padded job bound without increasing
    the supported context or allocating additional cache.
    """
    if not (
        _qc_decode_profile == "long" and _is_rocm
        and args.bsz == 1 and 1 <= args.q_len <= 8
        and args.dim == 256 and args.num_q_heads == 24 and args.num_kv_heads == 4
        and k_bits == 8 and v_bits == 8 and args.causal
        and args.cu_seqlens is None and args.sinks is None
        and args.window_size in (None, -1) and not args.softcap
        and 32768 <= min(args.block_table.shape[1], args.q_cache[0].shape[0]) * args.q_cache[0].shape[1] <= 120064
    ):
        return {}
    ...
    if _qc_device_arch[device] != "gfx1101":
        return {}
    options = {"block_n": 32, "num_splits": 32, "parallel_combine": True}
    if args.q_len >= 3:
...
@triton.jit
def _paged_attn_decode_combine_parallel_kernel(
    partial_o, partial_ml, out, h32, sinks,
    num_splits: tl.constexpr,
    QCV: tl.constexpr, HAS_SINKS: tl.constexpr,
    q_len: tl.constexpr, n_q_heads: tl.constexpr, n_kv_heads: tl.constexpr,
    head_dim: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_H: tl.constexpr,
    BLOCK_ROWS: tl.constexpr, BLOCK_SPLITS: tl.constexpr,
):
    """Reduce split results in parallel, one workgroup per live query/head.

    The split kernel's layout is unchanged. Unlike the inherited serial
    reduction, this reads only live rows and exposes head/query parallelism.
    """
    task = tl.program_id(0)
    q_head = task % n_q_heads
    row_q = (task // n_q_heads) % q_len
    batch = task // (n_q_heads * q_len)
    group_size = n_q_heads // n_kv_heads
    kv_head = q_head // group_size
    sibling = q_head % group_size
    pid = (batch * n_kv_heads + kv_head) * tl.cdiv(group_size, BLOCK_H) + sibling // BLOCK_H
    row = (sibling % BLOCK_H) * BLOCK_M + row_q

    split = tl.arange(0, BLOCK_SPLITS)
    live = split < num_splits
    ml_base = (pid * num_splits + split) * BLOCK_ROWS * 2 + row * 2
    m = tl.load(partial_ml + ml_base, mask=live, other=-float("inf"))
    l = tl.load(partial_ml + ml_base + 1, mask=live, other=0.0)
    maximum = tl.max(m, axis=0)
    if HAS_SINKS:
        sink = tl.load(sinks + q_head).to(tl.float32)
        maximum = tl.maximum(maximum, sink)
    safe_maximum = tl.where(maximum == -float("inf"), 0.0, maximum)
    weight = tl.where(m == -float("inf"), 0.0, tl.exp(m - safe_maximum))
    denominator = tl.sum(l * weight, axis=0)
    if HAS_SINKS:
        denominator += tl.exp(sink - safe_maximum)

    d = tl.arange(0, head_dim)
    po_base = ((pid * num_splits + split) * BLOCK_ROWS + row) * head_dim
    partial = tl.load(partial_o + po_base[:, None] + d[None, :], mask=live[:, None], other=0.0)
    value = tl.sum(partial * weight[:, None], axis=0) / tl.where(denominator == 0.0, 1.0, denominator)
    if QCV > 0:
        # Match the inherited H32 dot's fp16 input and fp16 matrix scale,
        # using a float32 butterfly within each 32-element group.
        value = value.to(tl.float16).to(tl.float32)
        for bit in tl.static_range(0, 5):
            peer = tl.gather(value, d ^ (1 << bit), axis=0)
            value = tl.where((d & (1 << bit)) == 0, value + peer, peer - value)
        value *= tl.load(h32).to(tl.float32)
    tl.store(out + task * head_dim + d, value)
...
def paged_attn_triton_decode(
    ...
    block_h = max(16 // block_m, 1) if head_block is None else head_block
    if not _is_power_of_2(block_h) or not 16 <= block_m * block_h <= 64:
        raise ValueError("head_block must produce a power-of-two tile of 16..64 rows")
    block_rows = block_m * block_h
    ...
    with torch.cuda.device(q.device):
        ...
        if num_splits > 1 and parallel_combine:
            _paged_attn_decode_combine_parallel_kernel[(bsz * q_len * n_q_heads,)](
                partial_o, partial_ml, out, h32, sinks,
                num_splits, qcv, has_sinks, q_len, n_q_heads, n_kv_heads, head_dim,
                block_m, block_h, block_rows, triton.next_power_of_2(num_splits),
                num_warps=8 if num_splits >= 32 else 4, num_stages=1,
            )
        elif num_splits > 1:
