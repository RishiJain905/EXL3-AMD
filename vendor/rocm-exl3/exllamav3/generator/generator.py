...
class Generator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.model = model
        shortlist = getattr(model, "quantlab_shortlist", None)
    ...
    def iterate_draftmodel_mtp_gen(self, results: list):
        ...
        if gpu_draft:
            ...
            if getattr(self, "quantlab_gpu_draft_metadata", False):
                block_index = block_index.to(temp_hidden.device)
                cache_seqlens = cache_seqlens.to(temp_hidden.device)
        for idx in range(window):
            params = {
            ...
            for idx, (job, a, b) in enumerate(zip(self.active_jobs, logit_mapping[:-1], logit_mapping[1:])):
                ...
                if getattr(self, "quantlab_batch_greedy", False):
                    ...
                    if can_batch_greedy(job, job_logits):
                        ...
                        # State updates, EOS, acceptance and rollback remain serial.
                        greedy_tokens = job.sampler.forward(
                            job_logits, rand_u32=0, tokenizer=self.tokenizer
                        ).cpu()
                ...
                for i in range(batch_logits.shape[1]):
                    ...
                    if greedy_tokens is None:
                        ...
                        next_token = greedy_tokens[:, i:i + 1]
