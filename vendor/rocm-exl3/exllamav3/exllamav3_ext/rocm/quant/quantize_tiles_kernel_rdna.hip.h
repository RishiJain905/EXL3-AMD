...
// =============================================================================
// quantize_tiles_kernel.cuh for RDNA -- include swaps only
// =============================================================================
//
// Generated mechanically from quant/quantize_tiles_kernel.cuh. The only change
// is codebook.cuh -> codebook_rdna.hip.h; the kernel body is untouched.
//
// Why a sibling is needed at all: codebook.cuh carries 10 inline-PTX `lop3.b32`
// blocks. Those use only "r" constraints, so they *parse* on amdgcn and fail
// during codegen instead -- reported as "illegal VGPR to SGPR copy" and
// "Unsupported instruction", which reads like a backend bug rather than inline
// PTX. codebook_rdna.hip.h implements the same LUT3 with the amdgcn intrinsic
// and is validated bit-exact over all 4096 inputs.
//
// __shfl_down_sync in the reduction is fine unmodified: hip_compat.hip.h maps it
// to the mask-free wave32 form, and the lanes are converged there.
//
// Regenerate with:
//   sed -e 's|"codebook.cuh"|"codebook_rdna.hip.h"|' \
//       -e 's|"../util|"../../util|g' quant/quantize_tiles_kernel.cuh
// =============================================================================
