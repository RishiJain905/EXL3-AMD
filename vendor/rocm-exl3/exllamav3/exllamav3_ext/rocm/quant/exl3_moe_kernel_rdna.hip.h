...
// =============================================================================
// exl3_moe_kernel.cuh for RDNA -- v1.3.0 structure, RDNA inner + RDNA barrier
// =============================================================================
//
// Generated from quant/exl3_moe_kernel.cuh. The kernel body is upstream v1.3.0
// verbatim; what changes is the include chain and one helper:
//
//   ../ptx.cuh                -> dropped. Upstream reaches it for exactly one
...
//   exl3_gemm_inner.cuh       -> exl3_gemm_inner_rdna.hip.h
//   exl3_kernel_map.cuh       -> exl3_kernel_map_rdna.hip.h
//
// The fork's own MoE kernel (rocm_exl3_legacy/.../exl3_moe_kernel_rdna.hip.h,
// 323 lines) was read end to end first. Almost none of it survives, and that is
// the correct outcome rather than a shortcut -- v1.3.0 changed the algorithm:
//
//   - Dynamic expert scheduling. The fork assigns experts round-robin
//     (`expert_idx_assign++ % concurrency != group_idx`); v1.3.0 draws tickets
//     from a self-resetting scheduler in the lock buffer so groups load-balance
//     greedily. Carrying the fork's static assignment forward would have been a
//     silent performance regression on skewed routing.
//   - A `cb` template parameter. The fork hardcodes cb = 1 (mcg). MoE has no
//     cb0 -- only cb1 (mcg) and cb2 (mul1) -- so the axis is 2 wide, not 3.
//   - Non-gated activations (MOE_ACT_RELU2_NOGATE). The fork predates it and
//     would run the gate GEMM unconditionally.
//   - `MIN(size_m, 16)` into the inner. The fork passes the unclamped `size_m`.
//   - Barrier placement: the fork syncs after every 16-row chunk inside the
//     GEMM loops; v1.3.0 syncs once after both g/u GEMMs.
//   - MOE_SH_STAGES is 3 upstream, 4 in the fork; MOE_SMS_PER_EXPERT is a
//     runtime `gridDim.x` upstream, a fixed 12 in the fork.
//
...
#include "exl3_kernel_map_rdna.hip.h"      // FIRST: establishes SMEM_MAX
#include "../../quant/exl3_moe_common.cuh"
#include "exl3_moe_shape_rdna.hip.h"   // AFTER common: overrides MOE_TILESIZE_K
#include "../../util.h"
#include "../../util.cuh"
...
// -----------------------------------------------------------------------------
// group_barrier -- inter-block sense-reversing barrier
// -----------------------------------------------------------------------------
// Upstream keeps this in ptx.cuh, which is misleading: the function contains no
// PTX. It is cuda::atomic_ref plus __nanosleep, both of which the ROCm shim
// already provides -- rocm/cuda_shim/cuda/atomic implements exactly the
// load/store/fetch_add slice it uses, and hip_compat.hip.h bridges __nanosleep.
// So this is upstream's logic verbatim, duplicated here only because the file it
// lives in cannot be included.
//
// The fork wrote its own equivalent with raw atomicAdd/atomicExch plus an
// explicit __threadfence() before the sense flip, noting that atomics are
// sequentially consistent on GFX11. That is the same barrier; upstream's
// acq_rel/release/acquire ordering expresses the requirement directly and the
// shim maps it onto __hip_atomic_* with agent scope, so the fence is implied
// rather than dropped. The fork's `__builtin_amdgcn_s_sleep` yield hint is what
// hip_compat's __nanosleep already compiles to.
//
// IMPORTANT: every block in the group must reach this. The MoE kernel is NOT
// launched cooperatively, so there is no runtime check on grid size -- if the
// group's blocks are not all resident, this deadlocks for real. Unlike the GEMM,
// where an oversubscribed cooperative grid is refused up front (see
// rocm_tools/gemm_coop_check.hip), nothing here will catch that mistake.
__device__ inline void group_barrier
(
    int group_id,
    int group_size,
    int* barrier_counters_sense  // length 2*max(group_id). odd positions are the sense bit
)
{
    __syncthreads();
...
// =============================================================================
// Fused MoE MLP kernel. Body is upstream v1.3.0.
//
